...
import { fromResourceObject, fromJsonApiData, toJsonApiDataIncluding, toNewResourceObject, toJsonApiData, JsonDeserializer, fromResourceObjectsOfType, toResourceObject, jsonSerializers } from '@moneybutton/json-api';
...
function getMoneyButtonClient(webStorage, webLocation) {
  ...
  class MoneyButtonClient {
    ...
    async logOut() {
      try {
        if (this.getRefreshToken()) {
          ...
          if (accessToken) {
            await fetch("".concat(this.mbUrl, "/oauth/v1/revoke"), {
              method: 'POST',
              headers: {
    ...
    async changePassword(password, encryptedMnemonic, xpub, language) {
      const loginPassword = await MoneyButtonClient._computeHmac256(LOGIN_PASSWORD_HMAC_KEY, password);
      const body = toJsonApiDataIncluding(toNewResourceObject('users', {
    ...
    requestAuthorization(scope, redirectUri, state = null) {
      if (typeof scope !== 'string' || scope.length === 0) {
        throw new Error("Invalid scope requested: ".concat(scope, "."));
      }

      if (typeof redirectUri !== 'string' || redirectUri.length === 0) {
        throw new Error("Invalid return URI: ".concat(redirectUri, "."));
      }
    ...
    _doAuthorizationCodeGrantAuthorizationRequest(redirectUri, scope, state = null) {
      ...
      const authorizationUri = ["".concat(this.mbUrl, "/oauth/v1/authorize"), queryString.stringify({
        response_type: 'code',
    ...
    async _doRefreshAccessTokenRequest(refreshToken) {
      const response = await fetch("".concat(this.mbUrl, "/oauth/v1/auth-proxy/token"), {
        method: 'POST',
        body: JSON.stringify({
    ...
    _buildBasicAuthHeaders() {
      ...
      const credentials = "".concat(this.clientId, ":").concat(this.clientSecret);
      return {
        Authorization: "Basic ".concat(Buffer.from(credentials).toString('base64'))
      };
    }
    /**
     * @private
    ...
    async _doAccessTokenRequest(body = {}, headers = {}) {
      const res = await fetch("".concat(this.mbUrl, "/oauth/v1/token"), {
        method: 'POST',
    ...
    async getClientByIdentifier(clientIdentifier) {
      const json = await this._doGetRequest("/v1/clients/client_identifier=".concat(clientIdentifier));
    ...
    async getAppProfileByOAuthIdentifier(oAuthIdentifier) {
      const json = await this._doGetRequest("/v1/application_profiles/oauth_identifier=".concat(oAuthIdentifier));
    ...
    async getAppProfileByClientIdentifier(oAuthIdentifier) {
      const json = await this._doGetRequest("/v1/application_profiles/client_identifier=".concat(oAuthIdentifier));
      return JsonDeserializer.deserialize(json);
    }
    /**
    ...
    async getUserByHandle(handle) {
      let json = await this._doGetRequest("/v1/users/handle/".concat(handle));
    ...
    async getUser(userId) {
      let json = await this._doGetRequest("/v1/users/".concat(userId));
      return JsonDeserializer.deserialize(json);
    }
    /**
    ...
    async getUserProfile(userId) {
      let json = await this._doGetRequest("/v1/users/".concat(userId, "/profile"));
      return JsonDeserializer.deserialize(json);
    }
    /**
    ...
    async updateUser(userId, attributes = {}) {
      ...
      const json = await this._doPatchRequest("/v1/users/".concat(userId), body);
    ...
    async getUserClients(userId, query = {}) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/clients"), query);
    ...
    async getUserUtxos(userId, query = {}) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/utxos"), query);
    ...
    async getUserUtxoById(userId, utxoId, query = {}) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/utxos/").concat(utxoId), query);
    ...
    async getUserApplications(userId, query = {}) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/applications"), query);
      return fromResourceObjectsOfType(fromJsonApiData(json), 'applications');
    }

    async getUserApplicationById(userId, appId, query = {}) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/applications/").concat(appId), query);
    ...
    async createUserApplication(userId, attributes) {
      ...
      const json = await this._doPostRequest("/v1/users/".concat(userId, "/applications"), body);
      return fromResourceObject(fromJsonApiData(json), 'applications');
    }
    /**
    ...
    async getCurrentUserSwipePermissionById(id) {
      const json = await this._doGetRequest("/v1/swipe_permissions/".concat(id));
      return JsonDeserializer.deserialize(json);
    }

    async updateCurrentUserSwipePermissionById(id, attributes) {
      const reqBody = SwipePermissionSerializer.serialize(attributes);
      const jsonResponse = await this._doPatchRequest("/v1/swipe_permissions/".concat(id), reqBody);
    ...
    async deleteCurrentUserSwipePermissionById(id) {
      await this._doDeleteRequest("/v1/swipe_permissions/".concat(id));
    }
    /**
    ...
    async updateUserApplication(userId, appId, attributes) {
      ...
      const json = await this._doPatchRequest("/v1/users/".concat(userId, "/applications/").concat(appId), body);
      return fromResourceObject(fromJsonApiData(json), 'applications');
    }
    /**
    ...
    async createUserClient(userId, attributes) {
      ...
      const json = await this._doPostRequest("/v1/users/".concat(userId, "/clients"), body);
      return fromResourceObject(fromJsonApiData(json), 'clients');
    }
    /**
    ...
    async updateUserClient(userId, clientId, attributes = {}) {
      const body = toJsonApiData(toResourceObject(clientId, 'clients', attributes));
      await this._doPatchRequest("/v1/users/".concat(userId, "/clients/").concat(clientId), body);
    }
    /**
    ...
    async getUserHandles(userId, query = {}) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/handles"), query);
    ...
    async updateUserHandle(userId, handleId, attributes = {}) {
      ...
      const json = await this._doPatchRequest("/v1/users/".concat(userId, "/handles/").concat(handleId), body);
    ...
    async checkHandleAvailability(search, query = {}) {
      let body = toJsonApiData(toNewResourceObject('handleChecks', {
        search
      }));
    ...
    async createUserHandle(userId, attributes) {
      ...
      const json = await this._doPostRequest("/v1/users/".concat(userId, "/handles"), body);
      return fromResourceObject(fromJsonApiData(json), 'handles');
    }
    /**
    ...
    async getUserWallet(userId, walletId) {
      ...
      if (data == null) {
        const json = await this._doGetRequest("/v1/users/".concat(userId, "/wallets/").concat(walletId));
    ...
    async getUserWallets(userId) {
      let json = await this._doGetRequest("/v1/users/".concat(userId, "/wallets/"));
    ...
    async createUserWallet(userId, attributes) {
      ...
      let json = await this._doPostRequest("/v1/users/".concat(userId, "/wallets"), body);
    ...
    async getBalance(userId) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/balance"));
      return JsonDeserializer.deserialize(json);
    }
    /**
    ...
    getWalletBalances(walletId = 'active') {
      return this._doGetRequest("/v2/me/wallets/".concat(walletId, "/balances"));
    }
    /**
    ...
    async getReceiveAddress(userId, walletId) {
      let json = await this._doPostRequest("/v1/users/".concat(userId, "/wallets/").concat(walletId, "/receive_address"));
      let {
        address
      } = fromResourceObject(fromJsonApiData(json), 'addresses');
      return address;
    }
    /**
    ...
    async getReceiveAddressSignature(userId, walletId) {
      let json = await this._doPostRequest("/v1/users/".concat(userId, "/wallets/").concat(walletId, "/receive_signed_address"));
      let {
        addressSignature,
        address
      } = fromResourceObject(fromJsonApiData(json), 'addresses');
    ...
    async getCurrencyAmount(userId, attributes) {
      ...
      const json = await this._doPostRequest("/v1/users/".concat(userId, "/currency"), body);
      const {
        amount,
        currency
      } = fromResourceObject(fromJsonApiData(json), 'currency');
      return {
        amount,
        currency
      };
    ...
    async getWalletBalance(userId, walletId) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/wallets/").concat(walletId, "/balance"));
    ...
    async updateWallet(userId, walletId, attributes) {
      const body = toJsonApiData(toResourceObject(walletId, 'wallets', attributes));
      await this._doPatchRequest("/v1/users/".concat(userId, "/wallets/").concat(walletId), body);
    }
    /**
    ...
    async getUserPayments(userId, paginate) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/payments?").concat(this._paginateUri(paginate)));
      return {
        pages: json.meta['total-pages'],
        payments: json.data.map(payment => fromResourceObject(payment, 'payments'))
      };
    }
    /**
    ...
    async getPaymentById(paymentId) {
      const json = await this._doGetRequest("/v1/payments/".concat(paymentId));
      return JsonDeserializer.deserialize(json);
    }
    /**
     * @private
    ...
    _paginateUri({
      ...
      const url = [];
      if (number) url.push("page[number]=".concat(number));
      if (size) url.push("page[size]=".concat(size));
      if (sort) url.push("sort=".concat(sort));
      return url.join('&');
    }
    /**
    ...
    async createUserPayment(userId, attributes, paymentOutputs, cryptoOperations = []) {
      return this._doPostRequest("/v2/me/payments", _objectSpread({}, attributes, {
        paymentOutputs,
        cryptoOperations
      }));
    }
    ...
    async estimatePaymentAmount(userId, attributes, paymentOutputs, cryptoOperations = []) {
      return this._doPostRequest("/v2/me/payments/estimate", _objectSpread({}, attributes, {
        paymentOutputs,
        cryptoOperations
      }));
    }
    ...
    async getUserPayment(paymentId) {
      return this._doGetRequest("/v2/me/payments/".concat(paymentId));
    }
    /**
    ...
    async createUserDeposit(userId, attributes) {
      ...
      const json = await this._doPostRequest("/v1/users/".concat(userId, "/deposits"), body);
    ...
    async getUserDeposit(userId, depositId) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/deposits/").concat(depositId));
      return fromResourceObject(fromJsonApiData(json), 'deposits');
    }
    /**
     *
    ...
    async createUserRecoverFullAmountPayment(walletId) {
      return this._doPostRequest("/v2/me/wallets/".concat(walletId, "/recover-full-amount"));
    }
    /**
    ...
    async broadcastPayment(_ref) {
      ...
      const body = payment;
      const json = await this._doPatchRequest("/v2/payments/".concat(id, "/broadcast"), body);
      return json;
    }
    /**
    ...
    async fetchUiData(userId) {
      const json = await this._doGetRequest("/v1/users/".concat(userId, "/ui-data"));
    ...
    async updateUiData(userId, data) {
      const json = await this._doPatchRequest("/v1/users/".concat(userId, "/ui-data"), toJsonApiData(toNewResourceObject('ui-data', data)));
      return fromResourceObject(fromJsonApiData(json), 'ui-data');
    }
    /**
    ...
    validateUserDomain(id) {
      return this._doPostRequest("/v2/me/domains/".concat(id, "/validate"));
    }
    /**
    ...
    deleteUserDomain(id) {
      return this._doDeleteRequest("/v2/me/domains/".concat(id));
    }
    /**
     * @private
    ...
    async _doRequest(endpoint, opts = {}, query = {}, accessToken = null) {
      const url = this._appendQuery("".concat(this.mbUrl, "/api").concat(endpoint), query);
      ...
      const res = await fetch(url, _objectSpread({}, opts, {
        headers
      }));
      let json = null;
    ...
    _appendQuery(url, query = {}) {
      ...
      if (page !== undefined) {
        for (const key in page) {
          queryWithoutPage["page[".concat(key, "]")] = page[key];
        }
      }

      return "".concat(url, "?").concat(queryString.stringify(queryWithoutPage, {
        arrayFormat: 'bracket'
      }));
    }
    /**
    ...
    */
    ...
    getStoredWalletData(userId, walletId) {
      const rawData = webStorage.getItem("".concat(STORAGE_NAMESPACE, ":").concat(userId, ":").concat(walletId, ":wallet_data"));
    ...
    saveWalletData(userId, walletId, walletData) {
      ...
      const key = "".concat(STORAGE_NAMESPACE, ":").concat(userId, ":").concat(walletId, ":wallet_data");
...
const MoneyButtonClient = getMoneyButtonClient(window.localStorage, window.location);

export { AuthError, MoneyButtonClient, RestError };
//# sourceMappingURL=moneybutton.client.browser.esm.js.map
